...
parser.add_argument("--pretrained_path", type=str, default=None, help="Path to pre-trained PPO model.")
parser.add_argument(
    "--keep_all_info",
    action="store_true",
    default=False,
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # 加载模型（如存在）
    if args_cli.pretrained_path is not None:
        model_path = os.path.join(args_cli.pretrained_path)
        vecnorm_path = os.path.join(args_cli.pretrained_path, "model_vecnormalize.pkl")

        # 加载环境向量归一化（如果存在）
        if os.path.exists(vecnorm_path):
            env = VecNormalize.load(vecnorm_path, env)
            env.training = True
            env.norm_reward = True  # 或 False，取决于你之前是否用了
            print("[INFO] Loaded VecNormalize parameters.")

        # 加载模型
        agent = PPO.load(model_path, env=env, tensorboard_log=log_dir)
        agent.set_env(env)
