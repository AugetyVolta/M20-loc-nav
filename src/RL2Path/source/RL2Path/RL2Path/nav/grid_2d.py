        ...
        for i in range(num_bots):
            ...
            for _ in range(max_trials):             
                ...
                if not (
                    ...
                    # print(f"机器人 {i} 采样的终点 ({gx:.2f}, {gy:.2f}) 超出边界，重新采样。")
                    continue

                # 障碍物碰撞检查
                if self.is_in_obstacle(gx, gy, safety_distance=safety_distance_goal):
                    # print(f"机器人 {i} 采样的终点 ({gx:.2f}, {gy:.2f}) 在障碍物内，重新采样。")
                    continue
