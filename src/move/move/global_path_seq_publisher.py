...
class GlobalPathSequencePublisher(Node):
    def __init__(self):
        ...
        self.declare_parameter("marker_label_z", 1.45)
        ...
        self.marker_timer = self.create_timer(2.0, self._publish_waypoints)
    ...
    def _request_plan_to_current_goal(self):
        ...
        future.add_done_callback(
            lambda done_future, version=request_version: self._goal_response_callback(done_future, version)
        )
    ...
    def _goal_response_callback(self, future, request_version: int):
        ...
        result_future.add_done_callback(
            lambda done_future, version=request_version: self._get_result_callback(done_future, version)
        )
    ...
    def _publish_empty_paths(self):
        ...
        if self.pure_pursuit_plan_pub is not None:
            self.pure_pursuit_plan_pub.publish(empty)
