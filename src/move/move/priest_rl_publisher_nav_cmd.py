...
class RLLocalPlannerNodeROS2(Node):
    def __init__(self):
        ...
        self.cfg = PlannerConfig(
            ...
            odom_frame=str(declare_get("odom_frame", "odom").string_value),
        ...
        if self.cfg.use_scan:
            ...
            self.sub_points = self.create_subscription(PointCloud, "points", self._on_points_pc, 10)
        else:
            self.sub_points = self.create_subscription(PointCloud, "points", self._on_points_pc, 10)
            self.sub_scan = None
        ...
        if self.priest_cfg.enable:
            ...
            if typ == "marker_array":
                self.sub_dyn = self.create_subscription(MarkerArray, t, self._on_dynamic_obstacles, 10)
            elif typ == "tracked_persons" and TrackedPersons is not None:
                self.sub_dyn = self.create_subscription(TrackedPersons, t, self._on_dynamic_obstacles, 10)
    ...
    def _compute_local_path_and_priest(self) -> Tuple[Optional[Path], Optional[Path]]:
        ...
        path_rl_for_publish[-1, 0] = float(goal_pp[0])
        path_rl_for_publish[-1, 1] = float(goal_pp[1])
