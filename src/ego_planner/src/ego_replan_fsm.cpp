...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    {
      auto create_goal_sub = [this](const std::string &topic)
      {
        return node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            topic,
            rclcpp::QoS(10),
      ...
      waypoint_sub_ = create_goal_sub(goal_topic);
      if (goal_topic != "/ego_goal_pose")
      {
        waypoint_ego_goal_sub_ = create_goal_sub("/ego_goal_pose");
      }
      if (goal_topic != "/move_base_simple/goal")
      {
        waypoint_move_base_sub_ = create_goal_sub("/move_base_simple/goal");
      }
      RCLCPP_INFO(node_->get_logger(), "EGO manual goal primary topic: %s", goal_topic.c_str());
    }
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      {
        // ROS2 nodes already owned by rclcpp::spin(node) cannot be spun again
        // inside a callback. If a new pure_pursuit goal arrives while EGO is
        // already generating/replanning a trajectory, keep the refreshed global
        // target and let the active FSM cycle consume it instead of blocking or
        // calling spin_some(node_), which throws:
        // "Node ... has already been added to an executor."
        RCLCPP_INFO(
            node_->get_logger(),
            "EGO updated target while FSM is busy; keep current planning state. state=%d, goal=(%.3f, %.3f, %.3f)",
  ...
  {
    if (msg->pose.position.z < min_goal_z_)
    {
      RCLCPP_WARN(
          node_->get_logger(),
          "Ignore EGO goal below min_goal_z: goal_z=%.3f, min_goal_z=%.3f",
          msg->pose.position.z,
          min_goal_z_);
      return;
    }
    ...
    if (!have_odom_)
    {
      RCLCPP_WARN(
          node_->get_logger(),
          "Ignore EGO goal because odom is not ready yet: frame=%s, xyz=(%.3f, %.3f, %.3f)",
          msg->header.frame_id.c_str(),
          msg->pose.position.x,
          msg->pose.position.y,
          msg->pose.position.z);
    ...
    RCLCPP_INFO(
        node_->get_logger(),
        "EGO accepted goal: frame=%s, odom=(%.3f, %.3f, %.3f), goal=(%.3f, %.3f, %.3f)",
        msg->header.frame_id.c_str(),
        odom_pos_(0),
        odom_pos_(1),
        odom_pos_(2),
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    case GEN_NEW_TRAJ:
    {
      ...
      else
      {
        RCLCPP_WARN(
            node_->get_logger(),
            "EGO failed to generate initial trajectory; wait for next target. odom=(%.3f, %.3f, %.3f), local_target=(%.3f, %.3f, %.3f), end=(%.3f, %.3f, %.3f)",
            odom_pos_(0),
            odom_pos_(1),
            odom_pos_(2),
            local_target_pt_(0),
